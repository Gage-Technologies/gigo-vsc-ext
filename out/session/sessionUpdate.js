...
async function executeLiveCheck(wsID, secret) {
    ...
    for (let i = 0; i < 3; i++) {
        try {
            ...
            let res = await axios_1.default.post("https://api.gigo.dev/internal/v1/ext/live-check", {
...
async function executeAfkCheck(wsID, secret, addMin) {
    ...
    for (let i = 0; i < 3; i++) {
        try {
            ...
            let res = await axios_1.default.post("https://api.gigo.dev/internal/v1/ext/afk", {
                // eslint-disable-next-line @typescript-eslint/naming-convention
                "workspace_id": wsID,
                "secret": secret,
                // eslint-disable-next-line @typescript-eslint/naming-convention
