...
class TutorialEditorProvider {
        ...
        webviewPanel.webview.onDidReceiveMessage(e => {
            switch (e.type) {
                ...
                case 'saveTourStep':
                    ...
                    var lines = [];
                    fs.readFile(path.join(this.baseWorkspaceUri.fsPath, parsedMsg.file), 'utf8', (err, data) => {
                        if (err) {
                ...
                case 'deleteTourStep':
                    ...
                    tss.steps = tss.steps.filter((_, index) => index !== deletedStepNum);
                    ...
                    const delTourData = yaml.load(deliFleContents);
                    const delStep = delTourData.steps.filter((step) => step.step_number === e.message);
    ...
    getHtmlForWebview(webview) {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'dist', 'tutorial-editor', 'catScratch.js'));
        const styleResetUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'dist', 'tutorial-editor', 'reset.css'));
        const styleVSCodeUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'dist', 'tutorial-editor', 'vscode.css'));
        const styleMainUri = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'dist', 'tutorial-editor', 'catScratch.css'));
        const styleJS = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'dist', 'tutorial-editor', 'style.js'));
...
exports.TutorialEditorProvider = TutorialEditorProvider;
TutorialEditorProvider.viewType = 'catCustoms.catScratch';
function getNonce() {
    let text = '';
